...
class DatabaseService
{
     ...
     * Get table indexes
     */
    public function getTableIndexes(string $table, string $connection = null): array
    {
        if (!$this->tableExists($table, $connection)) {
            return [];
        }
     ...
     * Get table sizes
     */
    public function getTableSizes(string $connection = null): array
    {
        $db = $this->getConnection($connection);
        $databaseName = $db->getDatabaseName();
     ...
     * Optimize table
     */
    public function optimizeTable(string $table, string $connection = null): bool
    {
        try {
            $db = $this->getConnection($connection);
     ...
     * Repair table
     */
    public function repairTable(string $table, string $connection = null): bool
    {
        try {
            $db = $this->getConnection($connection);
     ...
     * Check table
     */
    public function checkTable(string $table, string $connection = null): array
    {
        try {
            $db = $this->getConnection($connection);
     ...
     * Analyze table
     */
    public function analyzeTable(string $table, string $connection = null): bool
    {
        try {
            $db = $this->getConnection($connection);
     ...
     * Get slow queries
     */
    public function getSlowQueries(string $connection = null, int $limit = 100): array
    {
        try {
            $db = $this->getConnection($connection);
     ...
     * Get database status
     */
    public function getDatabaseStatus(string $connection = null): array
    {
        try {
            $db = $this->getConnection($connection);
     ...
     * Execute raw SQL
     */
    public function executeRaw(string $sql, array $bindings = [], string $connection = null): mixed
    {
        try {
            $db = $this->getConnection($connection);
     ...
     * Clear query log
     */
    public function clearQueryLog(string $connection = null): void
    {
        $db = $this->getConnection($connection);
        $db->flushQueryLog();
    }
