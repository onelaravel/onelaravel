"use strict";
(this["webpackChunkApp"] = this["webpackChunkApp"] || []).push([[554],{
...
// Initialize templates function

const initTemplates = function (__SYSTEM_DATA__ = {}) {
...
<a href="${App.View.escString(App.View.route('contact'))}">Liên hệ</a>
    ...
    return null;
        ...
        render: function(__$spaViewData$__ = {}) {
                let user = getUser(), ipAddress = getIpAddress();
        const chartState = updateStateByKey('chartState', chartData);
    let unassigned;
        const userState = updateStateByKey('userState', user);
    const {name, email} = user, test = null;
    const {title, description} = config, data = {"key": ["value", "demo"]};
    lockUpdateRealState();
