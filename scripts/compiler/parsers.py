...
class DirectiveParsers:
    ...
    def parse_vars(self, blade_code):
        ...
        return "let {" + ', '.join(var_parts) + "} = __$spaViewData$__ || {};"
    ...
    def parse_register(self, blade_code):
        ...
        register_match = re.search(r'@register(?:\s*\([^)]*\))?(.*?)@endregister', blade_code, re.DOTALL | re.IGNORECASE)
        if register_match:
            return register_match.group(1).strip()
            
        # Tìm @setup (alias của @register, có thể có hoặc không có parameters)
        setup_match = re.search(r'@setup(?:\s*\([^)]*\))?(.*?)@endsetup', blade_code, re.DOTALL | re.IGNORECASE)
        if setup_match:
            return setup_match.group(1).strip()
            
        # Tìm @script (xử lý như @register)
        script_match = re.search(r'@script\s*(?:\([^)]*\))?(.*?)@endscript', blade_code, re.DOTALL | re.IGNORECASE)
        if script_match:
    ...
    def parse_follow_directives(self, blade_code):
        """Parse @follow/@watch and @endfollow/@endwatch directives"""
        # Pattern to match @follow(...) ... @endfollow or @watch(...) ... @endwatch blocks
        # Note: @watch is an alias for @follow
        pattern = r'@(?:follow|watch)\s*\(([^)]*)\)(.*?)@end(?:follow|watch)'
        
        def replace_follow(match):
            params_expr = match.group(1).strip()
            content = match.group(2)  # Don't strip content to preserve whitespace
            
            # Parse parameters to extract state keys
            state_keys = self._parse_follow_parameters(params_expr)
            
            # Convert state keys to JavaScript array
            js_state_keys = json.dumps(state_keys)
            
            # Return the __follow call
            return f"${{this.__follow({js_state_keys}, () => `{content}`)}}"
        
        return re.sub(pattern, replace_follow, blade_code, flags=re.DOTALL | re.IGNORECASE)

    def _parse_follow_parameters(self, params_expr):
        """Parse @follow parameters to extract state keys"""
        if not params_expr.strip():
            return []
        
        # Handle different parameter formats:
        # 1. Single parameter: @follow($stateKey)
        # 2. Multiple parameters: @follow($stateKey, $stateKey2, ...)
        # 3. Array parameter: @follow([$stateKey, $stateKey2, ...])
        
        params_expr = params_expr.strip()
        
        # Case 1: Array parameter - @follow([$stateKey, $stateKey2, ...])
        if params_expr.startswith('[') and params_expr.endswith(']'):
            array_content = params_expr[1:-1].strip()
            if not array_content:
                return []
            
            # Split by comma and extract state keys
            state_keys = []
            parts = self._split_follow_parameters(array_content)
            for part in parts:
                part = part.strip()
                if part:
                    # Remove $ prefix and quotes
                    state_key = self._extract_state_key(part)
                    if state_key:
                        state_keys.append(state_key)
            return state_keys
        
        # Case 2: Multiple parameters or single parameter
        else:
            parts = self._split_follow_parameters(params_expr)
            state_keys = []
            for part in parts:
                part = part.strip()
                if part:
    ...
    def _split_follow_parameters(self, content):
        """Split follow parameters by comma, respecting parentheses, brackets and quotes"""
        parts = []
        current = ''
        paren_count = 0
        bracket_count = 0
    ...
    def _extract_state_key(self, param):
        """Extract state key from parameter, removing $ prefix and quotes"""
        param = param.strip()
    ...
    def parse_subscribe(self, blade_code):
        """Parse @subscribe/@dontsubscribe directive and return subscribe config
        Returns:
            - True/False for boolean subscription
            - ["key1", "key2", ...] for specific keys
            - None if no directive found (caller should default to True)
        """
        import re

        # dontsubscribe has highest priority if present anywhere
        if re.search(r'@dontsubscribe\b', blade_code, flags=re.IGNORECASE):
            return False

        # Find first @subscribe(
        match = re.search(r'@subscribe\s*\(', blade_code, flags=re.IGNORECASE)
        if not match:
            return None

        # Extract balanced parentheses content
        from template_processor import extract_balanced_parentheses
        start_pos = match.end() - 1
        content, _ = extract_balanced_parentheses(blade_code, start_pos)
        if content is None:
            return None

        expr = content.strip()
        # @all variants
        if re.match(r'^@?all$', expr, flags=re.IGNORECASE):
            return True
        # true/false
        if re.match(r'^(true|false)$', expr, flags=re.IGNORECASE):
            return expr.lower() == 'true'

        # Helper to split params respecting quotes/brackets
        def split_params(s):
            parts = []
            current = ''
            in_quotes = False
        ...
        if expr.startswith('[') and expr.endswith(']'):
            ...
            # If multiple params separated by comma
            if ',' in expr:
                parts = split_params(expr)
            else:
                parts = [expr]
        ...
        keys = [normalize_key(p) for p in parts if normalize_key(p)]
